...
@limiter.limit("10/minute")
async def google_callback(request: Request,callback: schemas.GoogleCallBack,platform: Literal["web", "mobile"] = Query("web"),
    ...
    return {"access_token": jwt_access_token,
            ...
            "user": {"id": user.id,
                     ...
                     "google_profile": {"name": google_user.get("name"),
...
async def link_google_account(request: Request,link_request: schemas.OAuthLink,current_user=Depends(JWTUtil.get_user),
    ...
    if exist_oauth and exist_oauth.user_id != current_user.id:
    ...
    logger.info("User linked Google account")
...
async def unlink_google_account(request: Request,current_user=Depends(JWTUtil.get_user),db: Session = Depends(get_db),):
    ...
    logger.info("User unlinked Google account")
