...
class OAuthAccount(Base):
    ...
    created_at=Column("created_at",DateTime(timezone=True),default=lambda:datetime.now(timezone.utc))
    updated_at=Column("updated_at",DateTime(timezone=True),default=lambda:datetime.now(timezone.utc),
...
class TokenBlackList(Base):
    ...
    id = Column("Id", Integer, primary_key=True)
    token = Column("token", String(500), nullable=False, index=True, unique=True) 
    blacklisted_at = Column("blacklisted_at", DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), index=True)
...
class OTP(Base):
    ...
    locked_until=Column("locked_until",DateTime(timezone=True),nullable=True)