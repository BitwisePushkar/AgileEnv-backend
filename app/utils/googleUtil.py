...
class GoogleOAuth:
    ...
    def get_authorization_url(
        ...
        """
        IMPORTANT:
        - Web  → Web client + web redirect
        - Mobile → Web client + mobile redirect
        """

        client_id = self.client_id_web

        redirect_uri = (
            self.redirect_uri_web
            if platform == "web"
            else self.redirect_uri_mobile
        )
    ...
    async def get_user_info(self, access_token: str) -> Optional[Dict]:
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Failed to get Google user info: {response.status_code}"
                )
                return None
