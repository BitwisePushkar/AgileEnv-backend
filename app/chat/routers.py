...
from app.chat.models import Chatroom, Message, Chattype
...
def format_message(message: Message, db: Session) -> schemas.MessageResponse:
    ...
    return schemas.MessageResponse(id=message.id, chat_id=message.chat_id, sender_id=message.sender_id,
                                   name=sname, photo=sphoto, content=message.content,deleted=message.deleted,
...
def format_chatroom(chatroom: Chatroom, db: Session) -> schemas.ChatroomResponse:
    members = [schemas.UserBasic(id=u.id,username=u.username,photo=u.profile.image_url if u.profile else None,)
               for u in chatroom.members]
    return schemas.ChatroomResponse(id=chatroom.id, name=chatroom.name, chat_type=chatroom.chat_type.value,
                                    workspace_id=chatroom.workspace_id, created_by=chatroom.created_by,created_at=chatroom.created_at,
                                    updated_at=chatroom.updated_at,members=members,)
...
@limiter.limit("20/minute")
def get_user_chats(request: Request, workspace_id: int,current_user: User = Depends(JWTUtil.get_user),
                   db: Session = Depends(get_db)):
...
@limiter.limit("20/minute")
def get_detail(request: Request, id: int,current_user: User = Depends(JWTUtil.get_user),
               db: Session = Depends(get_db)):
...
async def update_name(request: Request, id: int,name: str = Query(..., min_length=1, max_length=255, description="New chatroom name"),
    ...
    await manager.broadcast_to_chatroom(chatroom_id=id, message={"type": "chatroom_updated", "data": {"name": name}}, db=db,)
    return {"message": "Chatroom name updated successfully"}

@router.delete("/api/chat/delete/{id}/", status_code=status.HTTP_200_OK)
@limiter.limit("10/minute")
async def delete_chatroom(request: Request, id: int,current_user: User = Depends(JWTUtil.get_user),
                          db: Session = Depends(get_db)):
    chatroom = crud.get_chatroom(db, id)
    if not chatroom:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chatroom not found")
    ...
    if chatroom.created_by is not None and chatroom.created_by != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail="Only the chatroom creator can delete it")
    await manager.broadcast_to_chatroom(chatroom_id=id,message={"type": "chatroom_deleted", "data": {"chatroom_id": id}},
                                        db=db,)
    crud.delete_chatroom(db, id)
    return {"message": "Chatroom deleted successfully"}

@router.get("/api/chat/{id}/members/", response_model=List[schemas.ChatroomMemberResponse])
@limiter.limit("30/minute")
def get_chatroom_members(request: Request, id: int,current_user: User = Depends(JWTUtil.get_user),
                         db: Session = Depends(get_db)):
    chatroom = crud.get_chatroom(db, id)
    if not chatroom:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chatroom not found")
    ...
    members = crud.get_chatroom_members(db, id)
    return [schemas.ChatroomMemberResponse(id=m.user.id,username=m.user.username,photo=m.user.profile.image_url if m.user.profile else None,
...
async def add_member(request: Request, chatroom_id: int, user_id: int,current_user: User = Depends(JWTUtil.get_user),
    ...
    if chatroom.created_by != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Only the chatroom creator can add members")
...
def create_message(request: Request, id: int, message_data: schemas.Message,current_user: User = Depends(JWTUtil.get_user),
                   db: Session = Depends(get_db)):
    if not crud.is_member(db, id, current_user.id):
    ...
    return format_message(new_message, db)

@router.get("/api/chat/{chatroom_id}/messages/", response_model=List[schemas.MessageResponse])
@limiter.limit("60/minute")
def get_messages(request: Request, chatroom_id: int,limit: int = Query(50, ge=1, le=100, description="Number of messages to return"),
                 before_id: Optional[int] = Query(None, description="Return messages older than this message id"),
                 current_user: User = Depends(JWTUtil.get_user), db: Session = Depends(get_db)):
    if not crud.is_member(db, chatroom_id, current_user.id):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You are not a member")
    messages = crud.get_chat_messages(db=db, id=chatroom_id, limit=limit, before_id=before_id)
...
@limiter.limit("20/minute")
async def update_message(request: Request, message_id: int, message_data: schemas.Message,
                         current_user: User = Depends(JWTUtil.get_user), db: Session = Depends(get_db)):
...
@router.delete("/api/chat/message/delete/{message_id}/", status_code=status.HTTP_200_OK)
@limiter.limit("20/minute")
...
@router.post("/api/chat/{id}/read/", status_code=status.HTTP_200_OK)
@limiter.limit("120/minute")
def mark_as_read(request: Request, id: int,current_user: User = Depends(JWTUtil.get_user),
                 db: Session = Depends(get_db)):
    if not crud.is_member(db, id, current_user.id):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You are not a member")
    crud.mark_as_read(db, chatroom_id=id, user_id=current_user.id)
...
def get_unread_counts(request: Request, workspace_id: int,current_user: User = Depends(JWTUtil.get_user),
                      db: Session = Depends(get_db)):
    counts = crud.get_unread_counts(db, user_id=current_user.id, workspace_id=workspace_id)
    return counts
        ...
        try:
            while True:
                ...
                if message_type == "message":
                    ...
                    if not content:
                        continue
                    new_message = crud.create_message(db=db, chatroom_id=chatroom_id, id=current_user.id,
                                                      data=schemas.Message(content=content),)
                    response = format_message(new_message, db)
                    await manager.broadcast_to_chatroom(chatroom_id=chatroom_id,message={"type": "new_message", "data": response.model_dump()},
                                                        db=db,)
        except WebSocketDisconnect:
            manager.disconnect(current_user.id, chatroom_id, websocket)
