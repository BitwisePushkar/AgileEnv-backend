...
def get_user_chatrooms(db: Session, id: int, workspace_id: int) -> List[Chatroom]:
    return (db.query(Chatroom).join(ChatroomMember, Chatroom.id == ChatroomMember.chat_id)
            .filter(ChatroomMember.user_id == id, Chatroom.workspace_id == workspace_id).options(joinedload(Chatroom.members)).all())

def check_chat_exists(db: Session, id: int, other_id: int, workspace_id: int) -> Optional[Chatroom]:
    user_chat = (db.query(Chatroom).join(ChatroomMember, Chatroom.id == ChatroomMember.chat_id)
                 .filter(ChatroomMember.user_id == id,Chatroom.chat_type == Chattype.DIRECT,
...
def create_chatroom(db: Session, chatroom_data: ChatroomCreate, id: int) -> Chatroom:
    ...
    db.add(chatroom)
    db.flush()
    current_user_member = ChatroomMember(chat_id=chatroom.id, user_id=id)
    db.add(current_user_member)
...
def is_member(db: Session, chatroom_id: int, id: int) -> bool:
    return db.query(ChatroomMember).filter(ChatroomMember.chat_id == chatroom_id,ChatroomMember.user_id == id,).first() is not None
...
def get_chat_messages(db: Session,id: int,limit: int = 50,before_id: Optional[int] = None,) -> List[Message]:
    query = (db.query(Message).options(joinedload(Message.sender).joinedload(User.profile))
...
def create_message(db: Session, chatroom_id: int, id: int, data: MessageSchema) -> Message:
    ...
    db.commit()
    db.refresh(message)
    return (db.query(Message).options(joinedload(Message.sender).joinedload(User.profile)).filter(Message.id == message.id)
            .first())
...
def update_message(db: Session, id: int, data: MessageSchema) -> Optional[Message]:
    ...
    if message and not message.deleted:
        ...
        db.commit()
        return (db.query(Message).options(joinedload(Message.sender).joinedload(User.profile))
...
def mark_as_read(db: Session, chatroom_id: int, user_id: int) -> bool:
    latest = (db.query(Message.id).filter(Message.chat_id == chatroom_id).order_by(desc(Message.id)).first())
    if not latest:
        return False
...
def get_unread_counts(db: Session, user_id: int, workspace_id: int) -> List[dict]:
    ...
    for m in memberships:
        query = db.query(Message).filter(Message.chat_id == m.chat_id,Message.deleted == False, )
        if m.last_read_message_id is not None:
            query = query.filter(Message.id > m.last_read_message_id)
        result.append({"chatroom_id": m.chat_id, "unread_count": query.count()})
    return result