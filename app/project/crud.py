...
from app.workspace.crud import get_workspace_or_404
...
def is_workspace_admin(db: Session, workspace_id: int, user_id: int) -> bool:
    member = (db.query(WorkspaceMember).filter(WorkspaceMember.workspace_id == workspace_id,
...
def require_project_role(db: Session,project: Project,user_id: int,minimum_role: str,) -> None:
...
def create_project(db: Session,workspace_id: int,data: schemas.ProjectCreate,creator_id: int,) -> Project:
    project = Project(workspace_id=workspace_id,
                      name=data.name,
...
def add_project_member(db: Session,project_id: int,user_id: int,role: str = "viewer",) -> Tuple[ProjectMember, bool]:
...
def count_managers(db: Session, project_id: int) -> int:
    return (db.query(ProjectMember).filter(ProjectMember.project_id == project_id,
                                           ProjectMember.role == "manager",).count())